...
Сервис суммаризации текстов на основе LLM. Принимает произвольный текст через REST API,
формирует промпт, отправляет запрос к модели (совместимой с OpenAI API) и возвращает
краткое содержание.

## Возможности

- **REST API** на базе FastAPI с автоматической документацией (Swagger / ReDoc)
- **LLM-интеграция** — совместимость с любым сервером, имеющим OpenAI-совместимый API
- **Fallback-режим** — при недоступности LLM возвращает заранее заданное сообщение
- **Валидация входных данных** — проверка на пустоту и превышение максимальной длины
- **Логирование** — подробные логи с `request_id`, временем выполнения и статусами
...
4. **Настройте переменные окружения**
   ...
   Откройте `.env` и укажите параметры вашего LLM-сервера:

   | Переменная | Описание | По умолчанию |
   |---|---|---|
   | `LLM_BASE_URL` | URL LLM-сервера (OpenAI-совместимый) | `http://localhost:8000/v1` |
   | `LLM_MODEL` | Имя модели | `default-model` |
...
Принимает текст, возвращает его краткое содержание, сгенерированное LLM.
...
#### 4. Fallback-режим (LLM недоступен)

Если LLM-сервер не отвечает (соединение или таймаут), сервис возвращает fallback-сообщение:
...
│   └── settings.py
├── llm/              # Клиент LLM, промпты, пост-обработка
│   ├── client.py
│   ├── postprocess.py
│   └── prompts.py
...
| **Pipeline** | `services/pipeline.py` | Бизнес-логика: сборка промпта → вызов LLM → пост-обработка, fallback |
| **LLM Client** | `llm/client.py` | Асинхронный вызов модели, обработка таймаутов и сетевых ошибок |
...
4. **LLM Client** отправляет промпт на внешний сервер по OpenAI-совместимому API
...
7. При ошибке LLM (сеть / таймаут) возвращается **fallback-сообщение**, если `FALLBACK_ENABLED=true`
...
|---|---|---|
| `LLM_BASE_URL` | `http://localhost:8000/v1` | URL OpenAI-совместимого API |
| `LLM_MODEL` | `default-model` | Имя модели для генерации |
| `LLM_API_KEY` | `` | API-ключ (если требуется) |
| `LLM_TIMEOUT` | `60` | Таймаут запроса к LLM в секундах |
...
| Переменная | По умолчанию | Описание |
|---|---|---|
| `FALLBACK_ENABLED` | `true` | Включить fallback-режим при недоступности LLM |
| `FALLBACK_SUMMARY` | `Сервис суммаризации временно недоступен...` | Текст, возвращаемый вместо LLM |
